#!/usr/bin/env python3
"""Local runner for the stage MLflow tracking v3 export notebook.
...
BASE_URL_DEFAULT = (
    "https://apim-external-cub3dqcrgsdnebcb.a01.azurefd.net/benefits-stage/plan-list/"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Export stage MLflow tracking v3 + feedback rows locally."
    )
    ...
    parser.add_argument(
        ...
        default="STAGE_APP_APIM_SUB_KEY",
    ...
    parser.add_argument(
        "--output-dir",
        default=os.getenv("OUTPUT_DIR", "stage-export-output"),
    )
...
def format_feedback(item: dict[str, Any]) -> str | None:
    parts: list[str] = []
    feedback_type = item.get("feedback_type")
    if feedback_type not in (None, ""):
        parts.append(str(feedback_type))
...
def main() -> int:
    ...
    merged_rows = [
        {
            ...
            "feedback": feedback_lookup.get(feedback_join_key(item)),
        }
    ...
    csv_path = output_dir / "stage_mlflow_tracking_v3_export.csv"
    summary_path = output_dir / "stage_mlflow_tracking_v3_export_summary.json"
