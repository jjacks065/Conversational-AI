...
> Reusable multi-tenant conversational AI platform. First customer: BSC member chat. MVP: Sep 1 2026. Owner: Jason Jackson.
>
> Ship BSC through Stellarus primitives — NOT a vendor one-off. Every external runtime sits behind a Stellarus-owned adapter. That is the reusable value.
...
**Trust spine**: Auth0 PKCE → APIM validates → CCS mints RS256 context token → `ContextTokenGuard` re-verifies on every hop. `customer_slug` resolved at APIM/CCS, never client-supplied, drives all tenant scoping.
